...
In order to get an idea of the structure of the data, we make a binary decision tree based on the data.  We try to predict the participant's response by context, the first phoneme of the response, the type of turn the response was taken from and the participants' age and sex.
...
mA4 = glmer(
  answer ~ 1 + 
    (1 + context | ESource /partID) +
    (0 + responsePhoneme | ESource /partID) +
    (1 | contextSample) +  
    (1 | responseSample),  
  data = d,
  family = binomial,
...
hues2 = seq(15, 375, length = 2 + 1)
hues2 = hcl(h = hues2, l = 65, c = 100)[1:2]

sumStats2$shape = c("a",'b','c','a','b','c')
sumStats2$shape2 = c("a",'a','a','b','b','b')

main.plot <- ggplot(sumStats2,
	aes(x = responsePhoneme, y = mean, colour=context, shape=shape2)) +
  geom_point(size=5,show.legend=T) + #geom_line(aes(group=context)) +
...
main.plot2 <- ggplot(sumStats2,
	aes(x = context, y = mean, colour=responsePhoneme, shape=shape)) +
  geom_point(size=5,position=dodge) + 
  geom_line(aes(group=responsePhoneme), position=dodge) +
  geom_errorbar(aes(ymax=mean+ci, ymin=mean-ci), width=0.25, position=dodge) +
  xlab("Previous turn type (context)") +
  ylab("Proportion of 'Question' responses") +
  coord_cartesian(ylim=c(0,1)) + 
  scale_color_manual("First Phoneme",
                     labels=(c("None","Other","wh")),
                     values = (hues))+
  scale_shape_manual("First Phoneme",
                     labels=(c("None","Other","wh")),
                     values = (c(17,18,19))) +
