...
In order to get an idea of the structure of the data, we make a binary decision tree based on the data.  We try to predict the participant's response by context, the type of turn the response was taken from and the participants' age, sex and the type of English they speak (American, British, or other, which are the main groups).
...
```{r}
anova(conXrPh, trialXCon, trialXph, trXcoXph)
```
...
The model does not converge, so the statistics from this model are not reliable.  However, we can see that the estimates are very similar, suggesting that stimulus set does not have an impact on the main findings.

```{r}
cbind(fixef(trialXCon),fixef(stimSet))
```
...
anova(trial, context,rPhon, conXrPh, trialXCon)
```
\newpage
